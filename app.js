...
(function(){
  // 1) Guarantee modal is a direct child of <body>, even if placed elsewhere.
  document.addEventListener('DOMContentLoaded', () => {
    const modal = document.getElementById('svcModal');
    if (modal && modal.parentNode !== document.body) {
      document.body.appendChild(modal);  // move it to <body>
    }
  });

  // 2) Open/close logic (delegated; works with your existing cards)
  const modal    = document.getElementById('svcModal');
  if(!modal) return;
  ...
  document.addEventListener('click', (e)=>{
    ...
    if(more){
      ...
      const title = card?.dataset?.title || card?.querySelector('.svc-name')?.textContent || 'Details';
      const sel   = more.getAttribute('data-target');
      const src   = sel && document.querySelector(sel);
      openModal(title, src ? src.innerHTML : '');
    }
    if(e.target.closest('.svc-close') || e.target.dataset.close === '1'){ closeModal(); }
  });
